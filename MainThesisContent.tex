...
\]
% \[ 
% \forall P_i\in E, \exists  -P_i \in E
% \]
\begin{figure}[H]
    \centering
...
% \]
\subsection*{Addition:}
% Operation + is commutative meaning $\forall P_i, P_j \in E, P_i + P_j = P_j + P_i$. \\
...
\text{2.) if } $P_1 = P_2 \wedge y_{P_1},\ycor{2} \neq 0\wedge P_1,P_2 \neq \mathcal{O} $: \par
\[
...
% \[
% P_1 + P_2 = (m^2 - 2{x_{P1}}\text{ , } m(x_{P_1} - x_{P_3}) - y_{P_1})
% \]
...
\text{3.) if } $P_1 = P_2 \wedge \ycor{1},\ycor{2} = 0 \vee P_1 = -P_2$  \par
\[
...
% Bézout's theorem is the reason why every line intersects EC in exactly 3 points.
\begin{figure}[h!]
  \centering
\includegraphics[width=0.45\textwidth]{cubes_intersection_assymetric.png}
...
\forall A,B,C \in E, \text{ }-((A+B)+C)=-(A+(B+C))
\]
% Considering Bézout's theorem for curves and the fact that it takes exactly 9 points to define cubic curve, we can tell that group formed by EC is associative.
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.387\textwidth]{bezout_better_colors.png}
